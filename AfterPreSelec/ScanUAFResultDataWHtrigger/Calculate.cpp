...
int main(int argc,char ** argv){
    ...
    myrandom = new TRandom3(26);

    string InputYear(argv[3]);


    int year = 0 ; 
    if((InputYear.find("2016APV",0)) != string::npos) year = 20160;
    else if((InputYear.find("2016",0)) != string::npos) year = 2016;
    else if ((InputYear.find("2017",0)) != string::npos) year = 2017;
    else if ((InputYear.find("2018",0)) != string::npos) year = 2018;

    std::string json_path;
    if(year == 20160) json_path = "./btagging/2016preVFP_UL/btagging.json";
    else if(year == 2016) json_path = "./btagging/2016postVFP_UL/btagging.json";
    else if(year == 2017) json_path = "./btagging/2017_UL/btagging.json";
    else if(year == 2018) json_path = "./btagging/2018_UL/btagging.json";

    //correction::CorrectionSet::fromfile cset = correction::CorrectionSet::from_file(json_path);
    //correction cset = correction::CorrectionSet::from_file(json_path);
    //std::unique_ptr<correction::CorrectionSet, std::default_delete<correction::CorrectionSet>> cset = correction::CorrectionSet::from_file(json_path);

    //auto cset = correction::CorrectionSet::from_file(json_path);
    //cout<<"cset   "<<typeid(cset).name()<<endl;
    
    //correction::Correction::Ref sfs_btag_incl;
    //correction::Correction::Ref sfs_btag_incl_up;
    //correction::Correction::Ref sfs_btag_incl_down;
    //correction::Correction::Ref sfs_btag_comb;
    //correction::Correction::Ref sfs_btag_comb_up;
    //correction::Correction::Ref sfs_btag_comb_down;


    //sfs_btag_incl = cset->at("deepJet_incl");
    //sfs_btag_comb = cset->at("deepJet_comb");
    ...
    Int_t  bjetCategory[20];
    Int_t  nbjet;
    Int_t  bjethadronflav[20];
    Float_t bjetpt[20];
    Float_t bjeteta[20];
    ...
    traintree->Branch("VBSjet1pt",&VBSjet1pt,"&VBSjet1pt/F");
    traintree->Branch("VBSjet1eta",&VBSjet1eta,"&VBSjet1eta/F");
    traintree->Branch("VBSjet1phi",&VBSjet1phi,"&VBSjet1phi/F");
    traintree->Branch("VBSjet2pt",&VBSjet2pt,"&VBSjet2pt/F");
    traintree->Branch("VBSjet2eta",&VBSjet2eta,"&VBSjet2eta/F");
    traintree->Branch("VBSjet2phi",&VBSjet2phi,"&VBSjet2phi/F");
    ...
    testtree->Branch("VBSjet1pt",&VBSjet1pt,"&VBSjet1pt/F");
    testtree->Branch("VBSjet1eta",&VBSjet1eta,"&VBSjet1eta/F");
    testtree->Branch("VBSjet1phi",&VBSjet1phi,"&VBSjet1phi/F");
    testtree->Branch("VBSjet2pt",&VBSjet2pt,"&VBSjet2pt/F");
    testtree->Branch("VBSjet2eta",&VBSjet2eta,"&VBSjet2eta/F");
    testtree->Branch("VBSjet2phi",&VBSjet2phi,"&VBSjet2phi/F");
    ...
    while(infile>>read){
        ...
        readtotaltree->SetBranchAddress("VBSjet1pt",&VBSjet1pt);
        readtotaltree->SetBranchAddress("VBSjet1eta",&VBSjet1eta);
        readtotaltree->SetBranchAddress("VBSjet1phi",&VBSjet1phi);
        readtotaltree->SetBranchAddress("VBSjet2pt",&VBSjet2pt);
        readtotaltree->SetBranchAddress("VBSjet2eta",&VBSjet2eta);
        readtotaltree->SetBranchAddress("VBSjet2phi",&VBSjet2phi);
        ...
        readtotaltree->SetBranchAddress("total_nbjet",&nbjet);
        readtotaltree->SetBranchAddress("total_bjetbtagCategory",&bjetCategory);
        readtotaltree->SetBranchAddress("total_bjethadronFlavour",&bjethadronflav);
        readtotaltree->SetBranchAddress("total_bjetpt",&bjetpt);
        readtotaltree->SetBranchAddress("total_bjeteta",&bjeteta);
...
//        readtotaltree->SetBranchAddress("divide",&divide);
        ...
        for(int ii = 0 ; ii < Nentries ; ii++){
            ...
            double nloose = 0 ; 
            double nnotbj = 0;
/*            for(int jj = 0 ; jj < nbjet ; jj ++){
                if(bjetCategory[jj] == 1 ) nloose += 1.0;
                else if(bjetCategory[jj] == 2 ) nnotbj += 1.0;
                if(bjethadronflav[jj] == 0)
                    cout<<"btagging1   "<<bjeteta[jj]<<"   "<<sfs_btag_incl->evaluate({"central","L",bjethadronflav[jj],fabs(bjeteta[jj]),bjetpt[jj]})<<endl;
                else
                    cout<<"btagging2   "<<bjeteta[jj]<<"   "<<sfs_btag_comb->evaluate({"central","L",bjethadronflav[jj],fabs(bjeteta[jj]),bjetpt[jj]})<<endl;
            }
*/


            if(Wjetscore > 0.5 && Hbbscore > 0.5 && Wjetscore < 0.7) {
            weighted_number[13] += weight;
            weighted_error[13] += weight*weight;
            }else if(Wjetscore > 0.3 && Hbbscore > 0.5 && Wjetscore < 0.5){
                weighted_number[14] += weight;
                weighted_error[14] += weight * weight;
            }else if(Wjetscore > 0.7 && Hbbscore > 0.3 && Hbbscore < 0.5){
                weighted_number[15] += weight;
                weighted_error[15] += weight * weight;
            }
...
//            if(trans_E > 3000) continue;
    ...
    writedatafile<<"weighted_number_H05W0507            "<<weighted_number[13]<<endl;
    writedatafile<<"weighted_number_H05W0407            "<<weighted_number[14]<<endl;
    writedatafile<<"weighted_number_H0305W07            "<<weighted_number[15]<<endl;
    ...
    writedatafile<<"weighted_error_H05W0507             "<<sqrt(weighted_error[13])<<endl;
    writedatafile<<"weighted_error_H05W0407             "<<sqrt(weighted_error[14])<<endl;
    writedatafile<<"weighted_error_H0305W07             "<<sqrt(weighted_error[15])<<endl;
    writefile->cd();
    writefile->Write();
