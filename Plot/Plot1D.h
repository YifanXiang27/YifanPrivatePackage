    ...
    if(!DoSetYRange){
        for(int cir1 = 0 ; cir1 < totalplotnumber ; cir1 ++){
            ...
            if(cir1 == 0){
                ...
                intescalefactor[cir1] = 1;
            }else{
                inte2 = AClone->Integral();
                if(inte2 != 0)
                    intescalefactor[cir1] = inte1 / inte2;
            }
            if(DoNorm){
                if(cir1!= 0)
                    AClone->Scale(intescalefactor[cir1]);
            }
...
//            A_add->SetLineWidth(3);
        ...
        if(DoAddRatio) {
            for(int cir1 = 1 ; cir1 < totalplotnumber ; cir1 ++){
                TF1 *TFaddpad;
                std::string S_intescalefactor = std::to_string(intescalefactor);
                const char* C_intescalefactor = S_intescalefactor.c_str();
                TFaddpad = new TF1("TFaddpad",C_intescalefactor,A_den->GetBinLowEdge(0),A_den->GetBinLowEdge(NBinsX));
                if(cir1 == 1){
